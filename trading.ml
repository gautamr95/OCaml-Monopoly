...
let trade_prompt pl b : unit=
  ...
  let valid = List.fold_left (fun a x -> a && (get_holder x) = trade_player) true in
  if not valid req_list then Printf.printf "Invalid entry" 
    ...
    if money > (get_money trade_player) then Printf.printf "They cannot afford this\n" else
      ...
      if not valid offer_list then Printf.printf "Invalid entry" 
        ...
        if offer > get_money pl then Printf.printf "You cannot afford this\n" else
